...
export function FinancialTrajectoryDivergence() {
  ...
  // Problem, The function is very reusable, but it's API is hard
    const  goals:FinancialGoalResponse[]=resolveData<FinancialGoalResponse[], typeof FullAdviceSchema> ( 
      "fullAdvice", 
      FullAdviceSchema ,
      [defaultGoals],
      (parsedData)=>parsedData?.goalsAndInvestementsAdvice.goals
    );     
    const  financialTrajectoryDivergence:FinancialTrajectoryDivergence=resolveData<FinancialTrajectoryDivergence, typeof FullAdviceSchema>(
      "fullAdvice",
      FullAdviceSchema,// powerfull run time validation
      defaultFinancialTrajectoryDivergence,// pass  specific default data segment , not the whole default data for the entire UI
      (data)=>data?.goalsAndInvestementsAdvice.financialTrajectoryDivergence // naming it 'parsedData'  would expose internal  implmentation information,which makes overhead on the user of the API, he might not understand what 'parsed'  means, breaking  the intended abstraction.
    );    
